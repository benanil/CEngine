...
// Helper functions
static inline int bend_min(const int a, const int b) { return a > b ? b : a; }
static inline int bend_max(const int a, const int b) { return a > b ? a : b; }
...
BendDispatchList SSSBuildDispatchList(
    const float inLightProjection[4],
    ...
    int inWaveSize)
{
    BendDispatchList result = {0};

    if (inWaveSize <= 0)
        return result;

    /* Floating point division in the shader has a practical precision limit
			when the light is very far off screen. Clamp W for XY only. */
    float xy_light_w = inLightProjection[3];
    const float fp_limit = 0.000002f * (float)inWaveSize;

    if (xy_light_w >= 0.0f && xy_light_w < fp_limit)
        xy_light_w = fp_limit;
    else if (xy_light_w < 0.0f && xy_light_w > -fp_limit)
        xy_light_w = -fp_limit;
    ...
    result.LightCoordinate_Shader[2] = (inLightProjection[3] == 0.0f) ? 0.0f :
                                       (inLightProjection[2] / inLightProjection[3]);

    result.LightCoordinate_Shader[3] = (inLightProjection[3] > 0.0f) ? 1.0f : -1.0f;
    ...
    for (int q = 0; q < 4; ++q)
    {
        ...
        const int bounds[4] =
        {
            ...
            bend_max(
                0,
                ((q & 1) ? biased_bounds[2] : -biased_bounds[0]) +
                inWaveSize * (vertical ? 1 : 2) - 1
            ) / inWaveSize,

            bend_max(
                0,
                ((q & 2) ? biased_bounds[3] : -biased_bounds[1]) +
                inWaveSize * (vertical ? 2 : 1) - 1
            ) / inWaveSize
        };
        ...
        else if (q == 2) axis_delta = -biased_bounds[0] - biased_bounds[3];
        else if (q == 3) axis_delta = -biased_bounds[2] + biased_bounds[3];
        ...
        *disp2 = *disp;
        ...
        const bool disp_valid = (disp->WaveCount[1] > 0 && disp->WaveCount[2] > 0);
        const bool disp2_valid = (disp2->WaveCount[1] > 0 && disp2->WaveCount[2] > 0);

        if (!disp_valid && !disp2_valid)
        {
            result.DispatchCount -= 2;
        }
        else if (!disp_valid)
        {
            *disp = *disp2;

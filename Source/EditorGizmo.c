...
typedef struct GizmoMember_
{
    ...
    u32 groupIdx;
    u32 entityIdx;
    AX_ALIGN(16) float startPos[4];
    AX_ALIGN(16) float startRot[4];
    AX_ALIGN(16) float startScale[4];  // stored 0..1 form
    AX_ALIGN(16) float localCenter[4]; // group aabb center in entity local space
    AX_ALIGN(16) float startCenter[4]; // world aabb center
} GizmoMember;

enum { GIZMO_MAX_MEMBERS = 256 };
...
typedef struct GizmoState_
{
    ...
    AX_ALIGN(16) float startPivot[4];
    AX_ALIGN(16) float startPlaneHit[4]; // plane translate start, also the previous drag-plane hit for rotation
    AX_ALIGN(16) float planeDelta[4];
    AX_ALIGN(16) float rotateTangent[4]; // ring tangent at the grab point, world space
} GizmoState;
...
// stored 0..1 scale, the shader displays it x10
static void GizmoUnpackScale(u32 packed, float out[4])
{
    out[0] = (f32)(packed & 0x7FFu) / 2047.0f;
    out[1] = (f32)((packed >> 11u) & 0x7FFu) / 2047.0f;
    out[2] = (f32)((packed >> 22u) & 0x3FFu) / 1023.0f;
    out[3] = 0.0f;
}
...
static u32 GizmoCollectMembers(Scene* scene, v128f* outCenter)
{
    ...
    for (u32 t = 0; t < gizmoNumTargets; t++)
    {
        ...
        for (u32 g = 0; g < set->numGroups && gizmoNumMembers < GIZMO_MAX_MEMBERS; g++)
        {
            ...
            v128f localCenter = VecMulf(VecSetR(group->aabbMin[0] + group->aabbMax[0],
                                                group->aabbMin[1] + group->aabbMax[1],
                                                group->aabbMin[2] + group->aabbMax[2], 0.0f), 0.5f);
            v128f localExtent = VecMulf(VecSetR(group->aabbMax[0] - group->aabbMin[0],
                                                group->aabbMax[1] - group->aabbMin[1],
                                                group->aabbMax[2] - group->aabbMin[2], 0.0f), 0.5f);
            for (u32 e = 0; e < group->numEntities && gizmoNumMembers < GIZMO_MAX_MEMBERS; e++)
            {
                ...
                member->groupIdx = g;
                member->entityIdx = e;
                VecStore(member->startPos, entity->position);
                VecStore(member->startRot, VecNorm(UnpackQuaternionS16Norm1(entity->rotation)));
                GizmoUnpackScale(entity->scale, member->startScale);
                VecStore(member->localCenter, localCenter);

                AX_ALIGN(16) float s[4];
                GizmoUnpackScale(entity->scale, s);
                v128f worldScale = VecSetR(s[0] * 10.0f, s[1] * 10.0f, s[2] * 10.0f, 0.0f);
                v128f rotation = VecLoad(member->startRot);
                v128f center = VecAdd(QMulVec3V(VecMul(localCenter, worldScale), rotation), entity->position);
                VecStore(member->startCenter, center);
...
{
    AX_ALIGN(16) float arr[4];
    VecStore(arr, p);
    gizmoVerts[gizmoNumVerts++] = (ALineVertex){ arr[0], arr[1], arr[2], color };
}
...
{
    v128f pivot = VecLoad(gizmo.startPivot);
    ...
    for (u32 m = 0; m < gizmoNumMembers; m++)
    {
        ...
        v128f startPos    = VecLoad(member->startPos);
        v128f startRot    = VecLoad(member->startRot);
        v128f startCenter = VecLoad(member->startCenter);
        v128f localCenter = VecLoad(member->localCenter);

        if (gizmo.mode == GizmoMode_Translate)
        {
            v128f delta = gizmo.hotAxis >= GizmoAxis_PlaneX
                ? VecLoad(gizmo.planeDelta)
                : VecMulf(axes[gizmo.hotAxis], gizmo.dragStart);
            entity->position = VecAdd(startPos, delta);
        }
        else if (gizmo.mode == GizmoMode_Rotate)
        {
            AX_ALIGN(16) float axisArr[4];
            VecStore(axisArr, axes[gizmo.hotAxis]);
            float3 axis = { axisArr[0], axisArr[1], axisArr[2] };
            v128f deltaQ = QFromAxisAngle(axis, gizmo.dragStart);
            ...
            v128f worldScale = VecSetR(member->startScale[0] * 10.0f, member->startScale[1] * 10.0f,
                                       member->startScale[2] * 10.0f, 0.0f);
        ...
        else // scale
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                f32 axisFactor = (gizmo.hotAxis == GizmoAxis_Uniform || gizmo.hotAxis == i) ? factor : 1.0f;
                s[i] = Clampf32(member->startScale[i] * axisFactor, 0.0001f, 1.0f);
            }
            s[3] = 0.0f;
            entity->scale = PackXY11Z10UnormToU32(VecLoad(s));
...
bool EditorGizmoUpdate(Camera* camera)
{
    ...
    if (!gizmo.dragging)
    {
        ...
        VecStore(gizmo.startPivot, center);
    }
    else
        center = VecLoad(gizmo.startPivot);

    // every member outlines, the whole selection highlights
    {
        OutlineTarget outlines[GIZMO_MAX_MEMBERS];
        for (u32 m = 0; m < gizmoNumMembers; m++)
            outlines[m] = (OutlineTarget){ gizmoMembers[m].skinned, gizmoMembers[m].groupIdx, gizmoMembers[m].entityIdx };
        RendererSetOutlineTargets(outlines, gizmoNumMembers);
    }
    ...
    if (GetKeyDown(SDLK_LCTRL) && GetKeyPressed('f'))
    {
        ...
        VecStore(c, center);
        camera->position.x = c[0] - camera->Front.x * focusDistance;
        camera->position.y = c[1] - camera->Front.y * focusDistance;
        camera->position.z = c[2] - camera->Front.z * focusDistance;
        Camera_RecalculateView(camera);
    }

    v128f pickedRotation = VecLoad(gizmoMembers[0].startRot);
    ...
    if (gizmo.dragging)
    {
        ...
        {
            v128f startPivot = VecLoad(gizmo.startPivot);
            bool apply = false;
            ...
            if (gizmo.hotAxis >= GizmoAxis_PlaneX) // plane translate, free in two axes
            {
                ...
                {
                    VecStore(gizmo.planeDelta, VecSub(hitPoint, VecLoad(gizmo.startPlaneHit)));
                    apply = true;
                }
            }
            ...
            else if (gizmo.mode == GizmoMode_Rotate)
            {
                ...
                {
                    v128f hitDelta = VecSub(hitPoint, VecLoad(gizmo.startPlaneHit));
                    VecStore(gizmo.startPlaneHit, hitPoint);
                    f32 along = Vec3DotfV(hitDelta, VecLoad(gizmo.rotateTangent));
    ...
    else
    {
        ...
        f32 bestRef = 0.0f;
        AX_ALIGN(16) float planeHit[4] = { 0 };
        ...
        if (gizmo.hotAxis != GizmoAxis_None && GetMousePressed(MouseButton_Left) && !UIAnyWindowHovered())
        {
            ...
            gizmo.dragging = true;
            MemCopy(gizmo.startPlaneHit, planeHit, sizeof(planeHit));
            ...
            if (gizmo.mode == GizmoMode_Rotate)
            {
                ...
                VecStore(gizmo.rotateTangent, Vec3Cross(axes[gizmo.hotAxis], grabDir));

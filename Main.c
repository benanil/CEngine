...
void Init(void)
{
    ...
    for (int i = 0; i < NUM_ANIMS; i++)
    {
        ...
        AnimationController_Create(sceneBundle, &animController[i], true, 58, outMatrices);
    }
...
void Frame(void)
{
    ...
    for (int i = 0; i < NUM_ANIMS; i++)
    {
        ...
        AnimationController_SampleAnimationPose(ac, ac->mAnimPoseA, animIdx, animRatio);
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    const uint64_t halfMemory = TLSF_MEMORY_SIZE / 2;
    GlobalTLSF = TLSFCreateWithPool(AppMemory, halfMemory);
    GlobalArena.buf = AppMemory + halfMemory;
    GlobalArena.buffLen = halfMemory;

...
void WriteScreenSpaceShadow(struct DispatchParameters inParameters, int3 inGroupID, uint3 inGroupThreadID);
...
#ifndef BEND_SSS_SAMPLE_DEPTH
#define BEND_SSS_SAMPLE_DEPTH(inParameters, pixelXY) inParameters.DepthTexture.SampleLevel(inParameters.PointBorderSampler, (pixelXY) * inParameters.InvDepthTextureSize, 0)
#endif

#ifndef BEND_SSS_WRITE_OUTPUT
#define BEND_SSS_WRITE_OUTPUT(inParameters, pixelXY, value) inParameters.OutputTexture[(pixelXY)] = (value)
#endif
	...
	//	(int3)	inGroupID:			Compute shader group id register (SV_GroupID)
	//	(int)	inGroupThreadId:	Compute shader group thread id register (SV_GroupThreadID)
	void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, uint3 inGroupThreadID)
	{
		float2 xy_delta;
		float2 pixel_xy;
		...
		[unroll] for (i = 0; i < READ_COUNT; i++)
		{
			...
			depths.x = BEND_SSS_SAMPLE_DEPTH(inParameters, read_xy);
			depths.y = BEND_SSS_SAMPLE_DEPTH(inParameters, read_xy + offset_xy);
		#else
			int bias = bilinear > 0 ? 1 : -1;
			int2 offset_xy = int2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);

			depths.x = BEND_SSS_SAMPLE_DEPTH(inParameters, read_xy);
			depths.y = BEND_SSS_SAMPLE_DEPTH(inParameters, read_xy + float2(offset_xy));
		#endif
		...
		if (skip_pixel)
		{
			BEND_SSS_WRITE_OUTPUT(inParameters, (int2)write_xy, 1.0f);
			return;
		}

		float start_depth = sampling_depth[0];
		if (abs(start_depth - inParameters.FarDepthValue) <= 0.00001f)
		{
			BEND_SSS_WRITE_OUTPUT(inParameters, (int2)write_xy, 1.0f);
			return;
		}
		...
		//write the result
		{
			...
			if (inParameters.DebugOutputThreadIndex)
				result = (inGroupThreadID.x / (float)WAVE_SIZE);
			...
			BEND_SSS_WRITE_OUTPUT(inParameters, (int2)write_xy, result);
		}
	}
